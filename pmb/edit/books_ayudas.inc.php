...
// nombre de références par pages
	...
	if($check==1){
		switch($dest) {
			...
			default:
				...
				$ret_list="select el.expl_custom_champ, el.expl_custom_list_lib from expl_custom_lists el left join expl_custom ec on  el.expl_custom_champ=ec.idchamp where ec.name='Convocatoria' order by expl_custom_list_lib";		
				echo gen_liste($ret_list,"expl_custom_list_lib","expl_custom_list_lib","grant_period","",$grant_period,-1,"",0,$msg["select_grant_period"]);
	...
	if(!isset($nbr_lignes)) {
		...
		$res = @pmb_mysql_query($requete.$restrict_convo.$requete_unionall2.$restrict_convo.$requete_unionall3.$restrict_convo.$requeteb.$restrict_convo.$requeteb_unionall2.$restrict_convo.$requeteb_unionall3.$restrict_convo,$dbh);
	...
	if($nbr_lignes) {
		...
		switch($dest) {
			case "TABLEAU":
				...
				for($i=0; $i < pmb_mysql_num_rows($res); $i++) {
			...
			case "TABLEAUHTML":
				...
				while(($convo=pmb_mysql_fetch_object($res))) {
