...
// +-------------------------------------------------+
// $Id: print_thesaurus.php,v 1.27.6.3 2020/04/29 12:01:10 qvarin Exp $
			    ...
			    if($res && pmb_mysql_num_rows($res)){
			    	while ($categ=pmb_mysql_fetch_object($res)) {
						if(trim($categ->libelle_categorie)){
						    creer_categ_xml($dom,$noeudthes,0,$categ->num_noeud,$categ->libelle_categorie,$categ->note_application,$categ->comment_public,$categ->num_parent, $categ->autorite);
						}
					}
			    }
...
function creer_categ_xml($dom,$parent,$niveau, $num_noeud,$libelle_categorie,$note_application,$comment_public,$num_parent, $autorite){
	global $dbh, $aff_langue;
    ...
    if($aff_ts){
	    ...
	    if($res && pmb_mysql_num_rows($res)){
	    	...
	    	while ($categ=pmb_mysql_fetch_object($res)) {
				if(trim($categ->libelle_categorie)){
					creer_categ_xml($dom,$noeud_ts,($niveau+1),$categ->num_noeud,$categ->libelle_categorie,$categ->note_application,$categ->comment_public,$categ->num_parent, $categ->autorite);
				}
			}
	    }
    } 
