...
function update_docsloc_section(){
	...
	$result='0';
	$link2 = @pmb_mysql_connect(SQL_SERVER, USER_NAME, USER_PASS) OR die("Error MySQL");
	$sql_location="Select idlocation from docs_location";
	$select_location=@pmb_mysql_query($sql_location, $link2);
	$sql_section="Select idsection from docs_section";
	$select_section=@pmb_mysql_query($sql_section, $link2);
	$section_array=array();
	$location_array=array();
	while ($section=pmb_mysql_fetch_array($select_section)){
		$section_array[]=$section['idsection'];
								
	}
	while ($location=pmb_mysql_fetch_array($select_location)){
	...
	foreach ($section_array as $value){
		if ( $value=='28' ){
				foreach ($location_array as $value2){
					...
					$insert=@pmb_mysql_query($sql_locs_section,$link2);
