...
function show_import_choix_fichier($dbh,$from_ldap) {
	...
	// $result = pmb_mysql_query("Select duree_adhesion, libelle From empr_categ;") or die($msg["err_sql"]);
				...
				foreach($bufferLine as $dummykey=>$tmp){
					...
					if ( return_cb($dbh, $bufferChamp[${$desc_empr[1][0]}]) )	{
						...
						for($i = 1; $i < $nbChamp_empr; $i++){
							if(!$desc_empr[$i][4] and $bufferChamp[${$desc_empr[$i][0]}]){ // s'il n'est pas auto incr?mentable								
								...
								$res = pmb_mysql_query($query2) or print $msg["upd_echoue"]."<p>".$query2."<br />";
							}
						}
						...
						$res = pmb_mysql_query($query2) or print $msg["upd_echoue"]."<p>".$query2."<hr />";
						...
						$res = pmb_mysql_query($query2) or print $msg["upd_echoue"]."<p>".$query."<hr />";
						if ($from_ldap){
						...
						//print $query;
						$res = pmb_mysql_query($query) or print $msg["ins_echoue"]."<p>".$query."<hr />";
					...
					if ($pmb_lecteurs_localises) {
						if (!$empr_location || !$bufferChamp[$empr_location+1]) {
							...
							$res = pmb_mysql_query($req_location) or print $msg["upd_echoue"]."<p>".$req_location."<hr />";
						}
					}
					if(!$empr_login) {
						...
						$res = pmb_mysql_query($req_login) or print $msg["upd_echoue"]."<p>".$req_login."<hr />";
					}
						
					//Encryptage du mot de passe
					$query2 = 'select empr_login, empr_password from empr where empr_cb = "'.$bufferChamp[${$desc_empr[1][0]}].'" and empr_password_is_encrypted = 0';
					$res = pmb_mysql_query($query2) or print $msg["upd_echoue"]."<p>".$query2."<hr />";
					if(pmb_mysql_num_rows($res)) {
