...
// +-------------------------------------------------+
// $Id: SearchController.js,v 1.16.6.4 2020/10/27 16:10:47 qvarin Exp $
...
define(['dojo/_base/declare',
	...
	return declare(null, {
		...
		launchFilter : function() {
			...
			let children = this.store.getChildren({id : 'root'});
			children.forEach(element => {
				label = element.label.toLowerCase();
				
				smallChildrenFlag = false;
				smallChildren = this.store.getChildren(element);
				smallChildren.forEach(smallElement => {
					label = smallElement.label.toLowerCase();
					
					index = this.store.index[smallElement.id];
					if (label.indexOf(inputValue) != -1) {
						displayField = 'block';
					} else {
						displayField = 'none';
					}
					if (document.getElementById('dijit__TreeNode_' + index)) {
						if (displayField == 'block') {
							smallChildrenFlag = true;
						}
						document.getElementById('dijit__TreeNode_' + index).style.display = displayField;
					}
				});
				
				index = this.store.index[element.id];
				
				displayField = 'none';
				if (smallChildrenFlag) {
					displayField = 'block';
				}
				if (document.getElementById('dijit__TreeNode_' + index)) {
					document.getElementById('dijit__TreeNode_' + index).style.display = displayField;
