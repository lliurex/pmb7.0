...
// +-------------------------------------------------+
// $Id: rfid_pret.js,v 1.23.20.1 2020/02/04 10:08:51 ngantier Exp $
var count = 0;
...
function no_ack_antivol_pret(retVal) {
		alert ('La commande de l\'antivol n\'a pas r�pondu !');
...
function mode1_f_read_expl(cb,index,indexcount,antivol,uid) {	
	...
	// Montrer le bouton supprimer si pas pr�sent sur la platine
	...
	if (cb.length) { 
		// affecter liste cb d�j� trait� (pr�sent dans  liste_cb et lu sur la platine)
	...
	for (var i=0; i<tab_cb_nouveau.length; i++) {
		// cr�er nouveaux objects dans le tableau de pr�t
		mode1_one_more_ligne(tab_cb_nouveau[i]);
		// Ajout dans liste pmb � interroger
		tab_cb_pmb_request[i]=tab_cb_nouveau[i];
		// Ajout dans liste antivol � d�sactiver
	...
	// pour tous les cb d�j� trait�s (pr�sent dans  mode1_liste_cb et lu sur la platine)
	...
	// Si liste pmb � interroger non vide
	if(tab_cb_pmb_request.length){
		// Alerte activit� pmb
		mode1_activite_level++;
		flag_pmb_activite =1;
		mode1_flag_pmb_erreur = 0;
		// Requ�te serveur PMB pour lire infos expl et effectuer pr�ts temporaires de cette liste	
		mode1_do_pret_liste(tab_cb_pmb_request);
	}	
	
	// Si  liste antivol � d�sactiver non vide			
	if(mode1_tab_cb_antivol.length){					
		// Alerte activit� rfid		
		mode1_activite_level++;
		mode1_flag_rfid_activite = 1;
		mode1_flag_rfid_erreur = 0;
		// d�sactiver les antivols de cette liste
