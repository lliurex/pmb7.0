...
// +-------------------------------------------------+
// $Id: facette_search_opac.class.php,v 1.39.2.5 2020/08/05 09:09:36 btafforeau Exp $
...
class facette_search_opac {
	...
	public function get_display_list(){
		...
		if ($this->type == 'authperso') {
		    $tpl_vue_facettes = str_replace('!!authperso_name!!', '<th>' . $msg['admin_authperso_form_name'] . '</th>', $tpl_vue_facettes);
		} else {
		    $tpl_vue_facettes = str_replace('!!authperso_name!!', '', $tpl_vue_facettes);
		}
		...
		while($row = pmb_mysql_fetch_object($result)){
			...
			$lst .= "
				<tr class='".$pair_impair."'>
                    ...
                    <td ".$td_javascript.">";
			        $facette_critere = $row->facette_critere;
			        if ($facette_critere > $this->get_authperso_start() && $this->type != "authperso") {
					    ...
					    $lst .= "</td>
					             <td ".$td_javascript.">".(count($fields) > 1 ? htmlentities($fields[$facette_critere], ENT_QUOTES, $charset) : $msg["admin_opac_facette_ss_critere"])."</td>";
					} else {
					    $lst .= htmlentities($fields[$facette_critere],ENT_QUOTES,$charset);
					}
					$lst .= "
                    </td>
