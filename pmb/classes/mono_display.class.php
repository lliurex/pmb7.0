...
// +-------------------------------------------------+
// $Id: mono_display.class.php,v 1.326.2.8 2021/03/31 15:32:43 dgoron Exp $
...
//----------INICIO LLIUREX------		
		...
		*/
		...
		if($this->notice->code) {
			if(isISBN($this->notice->code)) {
				if ($zoneISBN) {
					$zoneISBN .= '</br><b>'.$msg['isbd_notice_isbn'].'</b> ';

				} else {
					$zoneISBN = '</br><b>'.$msg['isbd_notice_isbn'].'</b> ';
				}
			} else {
				$zoneISBN .= '.</br><b>'.$msg['notice_nonisbn'].'</b>';
			}
			$count++;
			$zoneISBN .= htmlentities($this->notice->code, ENT_QUOTES, $charset);
		}
		...
		if($this->notice->prix) {
			if($this->notice->code) {
				//--LLIUREX: $zoneISBN .= '&nbsp;: '.htmlentities($this->notice->prix, ENT_QUOTES, $charset);
				$zoneISBN .='';
				$count++;
			} else {
				if ($zoneISBN) 	{ 
	...
	//----FIN LLIUREX----------------------------------
		...
		if(count($this->langues)) {
			//--- LLIUREX: $langues .= "<b>${msg[537]}&nbsp;:</b> ".construit_liste_langues($this->langues);
			$langues .= '';
			$count++;

		}
		if(count($this->languesorg)) {
			//-- LLIUREX:$langues .= " <b>${msg[711]}</b>&nbsp;: ".construit_liste_langues($this->languesorg);
			$langues .= '';
			$count++;
		}
		if($langues)
