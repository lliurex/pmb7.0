...
// +-------------------------------------------------+
// $Id: docwatch_watch.class.php,v 1.54.2.4 2020/11/09 14:10:54 dgoron Exp $
			...
			foreach ($this->items as $item) {
				if ($item->get_interesting()) {
					if ($item->get_logo_url() != "") {
					    $image = "<img src='".$item->get_logo_url()."' class='align_right vignetteimg' hspace='4' vspace='2' style='max-width: 140px;max-height: 200px;'/>";
					} else {
						$image = "";
					}
					$publication_date="";
