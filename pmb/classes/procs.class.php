...
// +-------------------------------------------------+
// $Id: procs.class.php,v 1.22.2.5 2021/03/12 13:24:41 dgoron Exp $
			...
			if ((static::$module=='admin' && ($PMBuserid==1 || $row->autorisations_all || array_search ($PMBuserid, $rqt_autorisation)!==FALSE))
				...
				if ($class_prec!=$classement) {
					...
					if ($buf_tit) {
						...
						$buf_contenu.="
							<div class='row'>
								<input class='bouton_small' type='button' value=\"".$msg['704']."\" onClick=\"document.location='./admin.php?categ=proc&sub=proc&action=add&num_classement=".$buf_class."';\" />
								<input class='bouton_small' type='button' value=\"".$msg['procs_bt_import']."\" onClick=\"document.location='./admin.php?categ=proc&sub=proc&action=import&num_classement=".$buf_class."';\" />
							</div>";
						$maximise = false;
						if ($form_classement == $buf_class) {
							$maximise = true;
						}
						$display .= gen_plus("procclass".$buf_class,$buf_tit,$buf_contenu,$maximise);
						$buf_contenu="";
					}
