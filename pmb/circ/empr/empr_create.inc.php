...
//---------------------------LLIUREX 05/12/2019-----------------------------------//	
	//$rqt = "select max(CAST(empr_cb AS UNSIGNED))+1 as max_cb FROM (select empr_cb from empr UNION select COLLATE utf8_unicode_ci cb FROM empr_temp WHERE sess <>'".SESSid."') tmp";
	$rqt = "select max(CAST(empr_cb AS UNSIGNED))+1 as max_cb FROM (select empr_cb from empr UNION select cb COLLATE utf8_unicode_ci FROM empr_temp WHERE sess <>'".SESSid."') tmp";
