...
//----Changed calls to database. Used pmb_mysql instead mysql_ -----------------------
	$resultData = pmb_mysql_query($query, $dbh);
...
function getUsersCodes ($codigos, $dbh) {
	$matriz=array();
	...
	if (is_array($aux)) {
		foreach ($aux as $codi) {
			if(strpos($codi, "-") === false)
				...
				$aux2=explode("-",$codi);
				if ($aux2[0]<$aux2[1])  
					$m2=getCodesRango($aux2[0], $aux2[1], $dbh);
				else
					$m2=getCodesRango($aux2[1], $aux2[0], $dbh);	
...
$matriz=getUsersCodes($codigos,$dbh);		
