...
//--------------------LLIUREX 19/12/2018--------------------
//----Changed calls to database. Used pmb_mysql instead mysql------

$resultado_migracion=pmb_mysql_query($sql_migracion, $dbh);

if (pmb_mysql_num_rows($resultado_migracion)>0){

        
 	$sql_alumnos_migrados="SELECT empr_cb from empr where empr_Migrado='S'";
	$alumnos_migrados=pmb_mysql_query($sql_alumnos_migrados, $dbh);
	$total_migrados=pmb_mysql_num_rows($alumnos_migrados);
        $codigos=array();

         
        for ($i=0;$i<$total_migrados;$i++){
             $row =pmb_mysql_fetch_array($alumnos_migrados);
